...
window.onload = async () => {
  // Hides all of the editor UIs on load to avoid a mess
  $('[id$=Editor]').hide()

  // Load the JSON data into the editor and append the datasets to the dropdown on load
  const res = await fetch('/index/data.json')
  json = await res.json()
  for (const parent of Object.keys(json)) {
    dataSetSelector.innerHTML += `<option>${parent}</option>`
  }
}
