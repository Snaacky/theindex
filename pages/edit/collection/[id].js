...
export default function EditorCollection({
  ...
  if (_id !== '_new') {
    collection.columns = collection.columns.map((c) =>
      columns.find((t) => t._id === c)
    )
  ...
  return (
    <>
      ...
      <h4>Columns used in this collection</h4>
      {_id !== '_new' ? (
        <ColumnBoard
          _id={collection._id}
        ...
        <div className={'text-muted'}>
          Column selection will be available once the collection has been
          created
        </div>
      )}
    </>
