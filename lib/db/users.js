...
async function gatherUserInfo(uid, user) {
    ...
    const discordUser = await fetch("https://discord.com/api/users/" + data.providerAccountId, {
        headers: {
    ...
    if (data.providerAccountId && discordUser.avatar) {
        user.image = "https://cdn.discordapp.com/avatars/" + data.providerAccountId + "/" + discordUser.avatar + ".png"
    } else {
...
export async function updateUser(uid, {accountType, description, favs, lists, followLists}) {
    if (!uid) {
