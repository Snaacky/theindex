import {cleanId, count, deleteOne, find, findOne, getAll, getClient, insert, singularToPlural} from "./db"
...
export async function getLastViews(type, n) {
    ...
    try {
        ...
        data = await db.collection("views")
            .find({type})
            .sort({createdAt: -1})
            .limit(n)
            .toArray()
    ...
    let sorted = Object.keys(accumulated).map(id => {
        return {contentId: id, count: accumulated[id]}
    }).sort((a, b) => a.count > b.count ? -1 : 1)

    // resolve content info
    return await Promise.all(sorted.map(
        async s => await findOne(
            singularToPlural(type), type === "user" ? {uid: s.contentId} : {_id: s.contentId}
        )
    ))
}
